...
import { AuthContextProps, AuthContextType, AuthError } from "@/types/auth_types";
...
export default function AuthProvider({ children }: AuthContextProps) {
      ...
      async function getUserProfile(user: SupabaseUser) {
            ...
            if (!NetworkService.isOnline()) {
                  const networkError = ErrorService.createAndLog(
                        'network',
            ...
            try {
                  ...
                  if (profileError) {
                        const authError = ErrorService.handleSupabaseError(profileError, 'getUserProfile');
                        setError(authError);
                  ...
                  const unexpected_err = ErrorService.createErrorObject(
                        'generic',
                        'Unexpected Error.',
      ...
      const initializeAuth = useCallback(async () => {
            ...
            try {
                  ...
                  if (error) {
                        const authError = ErrorService.handleSupabaseError(error, 'initializeAuth');
                        setError(authError);
                        setUser(null);
                        setLoading(false);
                        navigate('/auth/login');
