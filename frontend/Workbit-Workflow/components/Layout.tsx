...
const NavItem: React.FC<{ icon: React.ReactNode; label: string; active?: boolean; path: string; }> = ({ icon, label, active, path }) => (
      <NavLink to={path}
...
export const Layout: React.FC<{currentPage: string; children: React.ReactNode}> = ({ currentPage, children }) => {
      ...
      return (
            <div className="min-h-screen bg-slate-50 flex">
                              ...
                              <nav className="flex-1 space-y-2">
                                    {navItems.map((item) => (
                                          <NavItem
                                                ...
                                                icon={item.icon}
                                                label={item.label}
                                                active={currentPage.includes(item.id)}
                                          />
                                    ))}
                              </nav>
                  ...
                  <main className="flex-1 min-w-0 flex flex-col h-screen overflow-hidden">
                        {/* Header */}
                        <header className="h-18 bg-white border-b border-slate-200 flex items-center justify-between px-6 shrink-0">
                        ...
                        <div className="flex-1 overflow-y-auto p-2">
                              <div className="max-w-8xl overflow-auto mx-auto space-y-8 pb-10">
                                    {children}
                              </div>
                        </div>
