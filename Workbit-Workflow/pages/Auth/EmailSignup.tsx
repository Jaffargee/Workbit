...
import { EmailAuthProps, ValidationError } from "@/types/auth_types";
...
const EmailSignup = () => {
      ...
      // State management
      const [formData, setFormData] = useState<EmailAuthProps>({ email: '' as Email, password: '' });
      ...
      const handleSignup = async (e: React.FormEvent<HTMLFormElement>) => {
            ...
            if (!NetworkService.isOnline()) {
                  ...
                  AnalyticsService.trackEvent('signup_failed', { reason: 'offline' });
                  return;
            }
            ...
            if (validationErrors.length > 0) {
                  ...
                  AnalyticsService.trackEvent('signup_validation_failed', { 
            ...
            try {
                  ...
                  // 5. Handle errors
                  if (error) {
                        const userFriendlyError = ErrorService.mapSupabaseError(error);
                        setErrors([{ field: 'general', message: userFriendlyError }]);
                        ErrorService.logError(error, 'EmailSignup:handleSignup');
                  ...
                  ErrorService.logError(error, 'EmailSignup:handleSignup:Unexpected');
                  AnalyticsService.trackEvent('signup_failed', { reason: 'unexpected_error' });
                  ...
                  <form onSubmit={handleSignup} className="bg-white rounded-[2.5rem] border-none max-sm:space-y-6" noValidate>
                        ...
                        <div className="flex flex-col items-center w-full relative space-y-6 md:gap-2">
                              ...
                              <div className="w-full">
                                    ...
                                    <div className="text-xs text-gray-500 mt-2 ml-2 space-y-1">
                                          <p>Password must contain:</p>
                                          <ul className="list-disc list-inside ml-2 space-y-0.5">
                                                <li className={requiredPasswordRequirements('passwd_len') && 'text-green-700'}>At least 8 characters</li>
                                                <li className={requiredPasswordRequirements('passwd_upper') && requiredPasswordRequirements('passwd_lower') && 'text-green-700'}>One uppercase and lowercase letter</li>
                                                <li className={requiredPasswordRequirements('passwd_number') && requiredPasswordRequirements('passwd_special') && 'text-green-700'}>One number and special character</li>
                                          </ul>
                                    </div>
                              </div>
