...
const Dashboard: React.FC<{ onPageChange: (p: string) => void }> = ({ onPageChange }) => {

      const { user, jobs, register, login } = useApp();

      const { profile, user: usr } = useAuth();

      const callback = useCallback(() => {
            register({ username: 'BbJafar', email: 'btechnology0000@gmail.com' });
            login('btechnology0000@gmail.com');
      }, []);
      ...
      return (
            <div className="space-y-8 animate-in fade-in duration-500">
                  ...
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-blue-600 p-8 rounded-[1rem] text-white overflow-hidden relative shadow-2xl shadow-blue-200">
                        <div className="absolute top-0 right-0 w-64 h-64 bg-white/10 rounded-full -mr-20 -mt-20 blur-3xl"></div>
                        <div className="relative z-10 space-y-2">
                              <h2 className="text-3xl font-bold tracking-tight">
                                    Welcome back, {user?.username}! 👋
                              </h2>
                              <p className="text-blue-100 font-medium">You have {jobs.length} new jobs waiting for you today.</p>
                        </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                        <StatCard
                              ...
                              value={`₦${user?.wallet.balance.toLocaleString()}`}
                              icon={<Wallet size={24} />}
                              color="text-blue-600"
                              bg="bg-blue-50"
                        />
                        <StatCard
                              ...
                              value={`₦${user?.wallet.earnings.toLocaleString()}`}
